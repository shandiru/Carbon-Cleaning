...
export default function ReviewsCarousel() {
  ...
  useEffect(() => {
    if (paused || !trackRef.current || reviews.length === 0) return;
    ...
    <section className="bg-black py-24 text-white overflow-hidden relative">
      ...
      <div className="relative w-full mx-auto mb-20"> {/* Added margin bottom */}
        <div 
          ...
          className="flex gap-8 px-4 overflow-x-auto scrollbar-hide scroll-smooth py-10"
        >
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#B62025] to-[#FF4B4B] flex items-center justify-center font-black text-white uppercase text-xl rotate-3 group-hover:rotate-0 transition-transform duration-300">
                    {review.name ? review.name[0] : "?"}
                  </div>
                  <div>
                    <p className="font-bold text-base tracking-tight text-white">{review.name}</p>
                    <p className="text-[#B62025] text-[10px] font-black uppercase tracking-[0.2em]">{review.date}</p>
                  </div>
                </div>
                
                <div className="flex text-yellow-500 gap-1 mb-6">
                   {[...Array(5)].map((_, idx) => (
                    <span key={idx} className="text-lg">{idx < review.rating ? '★' : '☆'}</span>
                  ))}
                </div>

                <p className="text-gray-400 text-sm italic leading-[1.8] font-medium line-clamp-4 group-hover:text-gray-200 transition-colors">
                  "{review.text}"
                </p>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
                ...
                <div className="flex items-center justify-center bg-[#B62025] w-7 h-7 rounded-full shadow-[0_0_15px_rgba(182,32,37,0.4)]">
